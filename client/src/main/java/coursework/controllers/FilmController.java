...
public class FilmController extends Open {
    ...
    @FXML
    private JFXTextField totalSeat;

    @FXML
    private JFXTextField totalRow;

    @FXML
    private JFXComboBox<Integer> numberHall;

    @FXML
    void onClicked_TableHall(MouseEvent event) {
        if (seanceTable.getSelectionModel().getSelectedItem() != null) {
    ...
    @FXML
    void onMouseClicked_UpdateSeat(MouseEvent event) throws RemoteException {
        fillingTableSeat();
    }

    @FXML
    void onMouseClicked_UpdateSeance(MouseEvent event) throws RemoteException {
        fillingTableSeance();
    }

    @FXML
    void onMouseClicked_UpdateFilm(MouseEvent event) throws RemoteException {
        fillingTableFilm();
    }
    ...
    void onAction_numberHall(ActionEvent event) throws RemoteException {
        ...
        totalSeat.setText(String.valueOf(seatList.get(0).getSeatTotal()));
        totalRow.setText(String.valueOf(seatList.get(0).getRowTotal()));
        } catch (NullPointerException e) { }
    }
    ...
    void onMouseClicked_View(MouseEvent event) throws IOException {
        session.setHall(seanceTable.getSelectionModel().getSelectedItem());
        buttonView.setDisable(false);
        initFXMLWindow(buttonView, "screen.fxml","Представление залла",0,0);
    }
    ...
    void onMouseClicked_addHall(MouseEvent event) {
        try {
            if (!nameHall.getText().equals("") || !totalRow.getText().equals("") || !totalSeat.getText().equals("")) {

                Integer.parseInt(totalRow.getText());
                Integer.parseInt(totalSeat.getText());
                ...
                notification.yesButton.setOnAction(ev -> {
                    ...
                    try {
                        ...
                        hall.setRowTotal(Integer.valueOf(totalRow.getText()));
                        hall.setSeatTotal(Integer.valueOf(totalSeat.getText()));
    ...
    void onMouseClicked_editHall(MouseEvent event) throws RemoteException {
        if(!nameHall.getText().equals("")) {
            ...
            notification.yesButton.setOnAction(ev -> {
                ...
                try {
                    client.EditHall(new Hall(numberHall.getValue(),nameHall.getText(), Integer.parseInt(totalRow.getText()),
                            Integer.parseInt(totalSeat.getText())));
    ...
    void initialize() throws IOException {
        ...
        searchSeat.textProperty().addListener((obs, oldText, newText) -> {
            if (!searchSeat.getText().isEmpty()) {
                ...
                {
                    if (seats.get(i).getId().getHallId().getId().equals(Integer.parseInt(searchSeat.getText().trim())))
                    {
